...
詳細な実行手順は Skill 側に寄せ、AGENTS.md では「何を守るか」と「どの Skill を使うか」を明確にする。
...
2. 参照ドキュメントの配置
- タスク設計ディレクトリ（`docs/tasks` または `docs/task-designs`）
...
6. Skill 配置方針（再現性）
- 新規開始をテンプレートリポジトリ（`.codex/skills` 同梱）にするか（既定: する）
- 空リポジトリ時のみグローバル bootstrap を初回1回だけ許容するか（既定: 許容する）
- 初回生成後に `<repo>/.codex/skills/` を正本化するか（既定: 正本化する）
...
- タスク設計の保存先は `docs/tasks` と `docs/task-designs` のどちらにしますか。
...
- 新規開始は `.codex/skills` 同梱テンプレートリポジトリで進めますか。
- 空リポジトリ時のみ、グローバル `python-project-bootstrap` を初回1回だけ使う運用でよいですか。
- 生成後は `<repo>/.codex/skills/` を正本に固定してよいですか。
...
- 長いコマンド列は AGENTS.md ではなく Skill に配置されている。
- 共有必須スキルの配置方針（repo 配下優先）が記載されている。
